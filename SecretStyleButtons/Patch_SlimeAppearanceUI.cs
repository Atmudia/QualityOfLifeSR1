...
{
    [HarmonyPatch(typeof(SlimeAppearanceUI))]
    public class Patch_SlimeAppearanceUI
    {
      public static Button AllDisableButton;
      public static Button AllEnableButton;
      [HarmonyPatch(nameof(Awake)), HarmonyPrefix]
      public static void Awake(SlimeAppearanceUI __instance)
      {
      ...
      [HarmonyPatch(nameof(ShowTutorial)), HarmonyPrefix]
      public static void ShowTutorial() => HideOrShowButtons(false);
      [HarmonyPatch(nameof(HideTutorial)), HarmonyPrefix]
      public static void HideTutorial() => HideOrShowButtons(true);
      public static void HideOrShowButtons(bool showButtons)
      {
        AllDisableButton?.gameObject.SetActive(showButtons);
        AllEnableButton?.gameObject.SetActive(showButtons);
      }
    }
}